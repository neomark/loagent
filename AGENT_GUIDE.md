...
### 1.3 코딩 전문 에이전트 (LOCoder) 사용하기
코드 작성 및 수정에 특화된 에이전트입니다.

```powershell
# 웹 UI (추천)
streamlit run app/main.py   # 실행 후 'Coding Chat' 메뉴 선택
...
- **일반 파일 확인**: "현재 디렉토리에서 `.py` 파일이 몇 개 있는지 셸 명령어로 확인해줘."

### 💻 LOCoder (코딩 에이전트) 활용 예시
LOCoder는 코드를 안전하게 수정하기 위해 반드시 `code_read`로 먼저 코드를 읽고, 수정한 뒤 `test_run`으로 검증하도록 설계되었습니다.
- **코드 수정 및 검증**: "config.py의 get_llm 함수에 timeout 파라미터를 추가하고 적용해줘."
- **버그 픽스**: "test_connection.py를 실행해보고, 오류가 나면 코드를 분석해서 고쳐줘."
- **프로젝트 파악**: "현재 프로젝트의 주요 구조를 확인하고 파일 목록을 요약해줘."
